(function (window) {
    ...
    var HZRecorder = function (stream, config) {
        ...
        this.getPCM = function (){
            this.stop();
            return floatTo16BitPCM(audioData.compress());
        };

        //回放
        this.play = function (audio) {
            audio.src = window.URL.createObjectURL(this.getBlob());
            // audio.src = window.URL.createObjectURL(new Blob([this.getPCM()], { type: 'audio/pcm' }));
        };

        //本地保存
        this.getLocal = function (){
            // return window.URL.createObjectURL(new Blob([this.getPCM()]));
